...
class CreateAdminsReservationsTimeSlotsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('admins_reservation_time_slot', function (Blueprint $table) {
            $table->increments('id');
            $table->timestamps();
            ...
            $table->foreign('time_slot_id')->references('id')->on('time_slots')->onDelete('cascade')->onUpdate
            ('cascade');
        });
    }
    ...
    {
        Schema::drop('admins_reservation_time_slot');
    }
}
